    ...
    **convergence_kwargs
    ...
    for iteration in torch.arange(0, max_iterations):
        ...
        for batch_idx, train_sample in enumerate(train_dataloader):
            ...
            xi = torch.linalg.inv(torch.t(thetas[0]) @ thetas[0]) @ torch.t(thetas[0]) @ time_derivs[0]
            
            
            #print(torch.round(xi, decimals=1))
            
            #reg_weights = LassoCV(cv=5, fit_intercept=False).fit(thetas[0].cpu().detach().numpy(), time_derivs[0].cpu().detach().numpy())
            
            #print(reg_weights.coef_)
            
            #print(reg_weights.score(thetas[0].cpu().detach().numpy(), time_derivs[0].cpu().detach().numpy()))
            
            #print(model.sparse_estimator_2(
            #    thetas, time_derivs
            #))
        ...
        if iteration % write_iterations == 0:
            ...
            #    iteration, torch.sum(mse), model, optimizer
            #)
            

            
            model.constraint.sparsity_masks = model.sparse_estimator_2(
                thetas, time_derivs)
